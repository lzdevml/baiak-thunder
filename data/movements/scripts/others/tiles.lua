...
function onStepIn(creature, item, position, fromPosition)
    ...
    if item.actionid >= 1000 then
        if creature:getLevel() < item.actionid - 1000 then
            ...
            creature:sendTextMessage(MESSAGE_INFO_DESCR, "The floor seems to be protected against unwanted invaders.")
        end
        return true
    end
    ...
    if Tile(position):hasFlag(TILESTATE_PROTECTIONZONE) then
        ...
        if depotItem then
            ...
            creature:sendTextMessage(MESSAGE_STATUS_DEFAULT, "Your depot contains " .. depotItems .. " item(s).")
            return true
        end
    end
