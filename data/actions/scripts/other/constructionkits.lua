...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if fromPosition.x == CONTAINER_POSITION then
        player:sendTextMessage(MESSAGE_STATUS_SMALL, "Place the construction kit on the ground first.")
    elseif not Tile(fromPosition):getHouse() then
        player:sendTextMessage(MESSAGE_STATUS_SMALL, "You can only construct this inside a house.")
    else
        item:transform(kit)
